...
//}
//void user::searchCourse_def(sqlite3* db, const char* db_path) {		//Nick Function
//	string dept;
...
//
//}
//void user::searchCourse(sqlite3* db, const char* db_path) { 	//Nick Function
//	string dept, sem;
//	//cout << "Enter course department: ";
//	cin >> dept;
...
//	sqlite3_stmt* stmt;
//
//	const char* sql = "SELECT CRN FROM COURSE WHERE DEPARTMENT = ? AND SEMESTER = ?";
//	int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr);
//	if (rc != SQLITE_OK) {
//		std::cerr << "Failed to prepare statement\n";
//	}
...
//
//	sqlite3_bind_text(stmt, 1, dept.c_str(), -1, SQLITE_TRANSIENT);
//	sqlite3_bind_text(stmt, 2, sem.c_str(), -1, SQLITE_TRANSIENT);
//
//	cout << "Courses in " << dept << " department and " << sem << " semester:\n";
//	while (sqlite3_step(stmt) == SQLITE_ROW) {
//		course* tempCourse = new course();
//		int code = sqlite3_column_int(stmt, 0);
//		populate_course(db, db_path, tempCourse, code);
